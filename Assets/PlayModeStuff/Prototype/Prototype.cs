    ...
    private void Update()
    {
    	...
    	while(commandQueue.TryDequeue(out cmd))
    	{
    		switch(cmd)
    		{
                ...
                case "capture":
                    GameObject captured = PointerController.instance.GetHitTarget(mobileID);
                    if (captured.name == "pobj_UFO")
                    {
                        ufo = captured.GetComponent<UFO>();
                        ufo.captured = true;
                    }
                    else if (captured.name == "pobj_Slime")
                    {
                        slime = captured.GetComponent<Slime>();
                        slime.captured = true;
                    }
                    else if (captured.name == "pobj_SpikeTrap")
                    {
                        spikes = captured.GetComponent<SpikeTrap>();
                        spikes.captured = true;
                    }
                    Debug.Log(captured);
                    Debug.Log(captured.transform.position.x);
                    PointerController.instance.MakeInvisible(mobileID);
                    PointerController.instance.Capture(mobileID);
                    break;  
                case "leave_ufo":
                    ufo.captured = false;
                    PointerController.instance.MakeVisible(mobileID);
                    PointerController.instance.Leave(mobileID);
                    break;
                case "leave_slime":
                    slime.captured = false;
                    PointerController.instance.MakeVisible(mobileID);
                    PointerController.instance.Leave(mobileID);
                    break;
                case "leave_spike":
                    spikes.captured = false;
                    PointerController.instance.MakeVisible(mobileID);
                    PointerController.instance.Leave(mobileID);
                    break;
                case "left":
